...
def get_rules_of_decision_tree(dt, feature_names = None, percent_threshold = 0.1, proportion_threshold = 0.8):
    """
    ...
    def recurse(left, right, threshold, features, node, depth = 0, parent = '', sign = '', path = []):
        ...
        if percent >= percent_threshold and max(propotions[0]) > proportion_threshold and node > 0 and rule != '':
